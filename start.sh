#!/usr/bin/env bash
# =============================================================================
# ComfyUI auto-deploy bootstrap for RunPod (no Docker, no registry).
# -----------------------------------------------------------------------------
# Everything installs onto the /workspace VOLUME (venv, ComfyUI, frontend),
# so the FIRST boot provisions (~15-20 min: torch + ComfyUI + frontend build)
# and EVERY LATER boot starts in seconds because the volume persists and every
# step is idempotent. cu128 torch runs on Ampere -> Blackwell (3090 ... 5090).
#
# Keep the SAME volume between pod stop/start to keep the fast reboots.
...
install_system_deps(){
  if ! command -v git >/dev/null 2>&1 || ! command -v aria2c >/dev/null 2>&1 \
     || ! command -v unzip >/dev/null 2>&1 || ! command -v python3 >/dev/null 2>&1; then
    log "Installing system packages (python3, git, aria2, wget, curl, unzip)..."
    export DEBIAN_FRONTEND=noninteractive
    apt-get update -y
    apt-get install -y --no-install-recommends \
      python3 python3-venv python3-dev \
      git aria2 wget curl ca-certificates unzip
  fi
}
...
download_models(){
  ...
  [ -f "$list" ] || return 0
  log "Downloading models (background)"
...
main(){
  ...
  setup_frontend
  install_custom_nodes
  # Models load on-demand at workflow run, not at startup -> download in the
  # background so the UI is reachable as soon as ComfyUI boots.
